...
<body>
    <nav class="navbar navbar-expand-md navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarsExample04">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item <?= (session('menu_customer') == 'home') ? 'active' : ''; ?>">
                    <a class="nav-link" href="{{ URL('/') }}">Home</a>
                </li>
                @if(!session('customer_id'))
                <li class="nav-item <?= (session('menu_customer') == 'login') ? 'active' : ''; ?>">
                    <a class="nav-link" href="{{ URL('/customer-login') }}">Login</a>
                </li>
                <li class="nav-item <?= (session('menu_customer') == 'register') ? 'active' : ''; ?>">
                    <a class="nav-link" href="{{ URL('/customer-register') }}">Register</a>
                </li>
                @else
                <li class="nav-item dropdown <?= (session('menu_customer') == 'user_menu') ? 'active' : ''; ?>">
                    <a class="nav-link dropdown-toggle" href="#" id="dropdown04" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{ $data_cutomer->username }}</a>
                    <div class="dropdown-menu" aria-labelledby="dropdown04">
                        <a class="dropdown-item" href="{{ URl('my-cart') }}">My Cart</a>
                        <a class="dropdown-item" href="{{ URl('history-transaction') }}">Transaction History</a>
                        <a class="dropdown-item" href="{{ URl('customer-change-password') }}">Change Password</a>
                        <a class="dropdown-item" href="{{ URl('customer-logout') }}">Logout</a>
                    </div>
                </li>
                @endif
                <li class="nav-item dropdown <?= (session('menu_customer') == 'categories') ? 'active' : ''; ?>">
    ...
    <script type="text/javascript">
        ...
        function ubah_qty(id) {
            var qty = "qty_" + id;
            var value1 = $("#qty_" + id).val();
            $.ajax({
                type: 'GET',
                url: "{{ URL('my-cart-update') }}" + "/" + id + "/" + value1,
